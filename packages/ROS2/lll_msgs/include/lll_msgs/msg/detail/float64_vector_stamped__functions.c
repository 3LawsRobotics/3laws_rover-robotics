  ...
  lll_msgs__msg__Float64VectorStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    lll_msgs__msg__Float64VectorStamped * data =
      (lll_msgs__msg__Float64VectorStamped *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!lll_msgs__msg__Float64VectorStamped__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          lll_msgs__msg__Float64VectorStamped__fini(&data[i]);
        }
        free(data);
        return false;
      }

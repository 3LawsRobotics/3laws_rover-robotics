  ...
  lll_rdm_msgs__msg__IncidentLogMsgData__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    lll_rdm_msgs__msg__IncidentLogMsgData * data =
      (lll_rdm_msgs__msg__IncidentLogMsgData *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!lll_rdm_msgs__msg__IncidentLogMsgData__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          lll_rdm_msgs__msg__IncidentLogMsgData__fini(&data[i]);
        }
        free(data);
        return false;
      }
